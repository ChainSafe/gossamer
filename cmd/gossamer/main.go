...
var (
	...
	importStateCommand = cli.Command{
		...
		Description: "The import-state command allows a JSON file containing a given state in the form of key-value pairs to be imported.\n" +
			...
			"\tUsage: gossamer import-state state.json\n",
	}
)
...
func importStateAction(ctx *cli.Context) error {
	arguments := ctx.Args()
	if len(arguments) < 3 {
		return fmt.Errorf("no args provided, please provide JSON state file, JSON header, and first slot number of the network")
	}

	stateFP := filepath.Clean(arguments[0])
	headerFP := filepath.Clean(arguments[1])
	firstSlot, err := strconv.Atoi(arguments[2])
	if err != nil {
		return err
	}
