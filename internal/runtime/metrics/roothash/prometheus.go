...
func NewPrometheus() (metrics *Prometheus, err error) {
	...
	metrics.nodeAdditions = prometheus.NewCounter(prometheus.CounterOpts{
		...
		Name:      "temp_root_tries_node_additions",
		Help:      "additions of nodes in all the temporary tries in memory used to calculate root hash",
	})
	collectorsToRegister["temporary root tries node additions counter"] = metrics.nodeAdditions

	metrics.nodeDeletions = prometheus.NewCounter(prometheus.CounterOpts{
		Namespace: "gossamer_runtime",
		Name:      "temp_root_tries_node_deletions",
		Help:      "deletions of nodes in all the temporary tries in memory used to calculate root hash",
	})
	collectorsToRegister["temporary root tries node deletions counter"] = metrics.nodeDeletions
