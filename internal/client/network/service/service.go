...
type NetworkPeers interface {
	...
	// local peer id).
	SetReservedPeers(protocol network.ProtocolName, peers map[string]any) error
	...
	// Returns an error if one of the given addresses is invalid or contains an invalid peer id (which includes the
	// local peer id).
	AddPeersToReservedSet(protocol network.ProtocolName, peers map[string]any) error
	...
	// Returns an error if one of the given addresses is invalid or contains an invalid peer id (which includes the
	// local peer id).
	AddToPeersSet(protocol network.ProtocolName, peers map[string]any) error
	// Remove peers from a peer set.
	//
