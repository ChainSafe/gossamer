...
func makePrecommit(t *testing.T,
	...
	encoded := primitives.NewLocalizedPayload(primitives.RoundNumber(round), primitives.SetID(setID), precommit)
...
func TestJustificationEncoding(t *testing.T) {
	...
	expAncestries := make([]*generic.Header[uint64, hash.H256, runtime.BlakeTwo256], 0)
	...
	justification, err := DecodeJustification[hash.H256, uint64, runtime.BlakeTwo256, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]](encodedJustification)
	require.NoError(t, err)
...
func TestDecodeJustificationFromWestendBlock2323464(t *testing.T) {
	...
	justification, err := DecodeJustification[hash.H256, uint32, runtime.BlakeTwo256, *generic.Header[uint32, hash.H256, runtime.BlakeTwo256]](encodedJustification)
	require.NoError(t, err)
...
func TestDecodeGrandpaJustificationVerifyFinalizes(t *testing.T) {
	...
	_, err := DecodeGrandpaJustificationVerifyFinalizes[hash.H256, uint64, runtime.BlakeTwo256, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]](
		invalidEncoding,
	...
	// Invalid target
	justification := primitives.GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
		Commit: primitives.Commit[hash.H256, uint64]{
			TargetHash:   a,
			TargetNumber: 1,
		},
	...
	hederList := []*generic.Header[uint64, hash.H256, runtime.BlakeTwo256]{headerB}
	...
	expectedJustification := primitives.GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
		Round: 1,
		Commit: primitives.Commit[hash.H256, uint64]{
			TargetHash:   a,
			TargetNumber: 1,
	...
	newJustification, err := DecodeGrandpaJustificationVerifyFinalizes[hash.H256, uint64, runtime.BlakeTwo256, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]](
...
func TestJustification_verify(t *testing.T) {
	...
	justification := GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{}
	...
	validJustification := GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
		Justification: primitives.GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
			Round: 1,
			Commit: primitives.Commit[hash.H256, uint64]{
				TargetHash:   a,
...
func TestJustification_verifyWithVoterSet(t *testing.T) {
	...
	invalidJustification := GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
		primitives.GrandpaJustification[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
			Commit: primitives.Commit[hash.H256, uint64]{
				TargetHash:   "B",
				TargetNumber: 2,
			},
	...
	headerList := []*generic.Header[uint64, hash.H256, runtime.BlakeTwo256]{
		headerA,
		headerB,
	}
...
func Test_newAncestryChain(t *testing.T) {
	...
	expAncestryMap := make(map[hash.H256]*generic.Header[uint64, hash.H256, runtime.BlakeTwo256])
	expAncestryMap[dummyHeader.Hash()] = dummyHeader
	type testCase struct {
		name    string
		headers []*generic.Header[uint64, hash.H256, runtime.BlakeTwo256]
		want    ancestryChain[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]
	}
	tests := []testCase{
		{
			name:    "noInputHeaders",
			headers: []*generic.Header[uint64, hash.H256, runtime.BlakeTwo256]{},
			want: ancestryChain[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]{
				ancestry: make(map[hash.H256]*generic.Header[uint64, hash.H256, runtime.BlakeTwo256]),
			},
		},
		{
			name: "validInput",
...
func TestAncestryChain_Ancestry(t *testing.T) {
	...
	invalidHeaderList := []*generic.Header[uint64, hash.H256, runtime.BlakeTwo256]{
		invalidParentHeader,
	}
	...
	type testCase struct {
		name   string
		chain  ancestryChain[hash.H256, uint64, *generic.Header[uint64, hash.H256, runtime.BlakeTwo256]]
		base   hash.H256
		block  hash.H256
		want   []hash.H256
		expErr error
