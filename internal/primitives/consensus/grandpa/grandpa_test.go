...
func TestCheckMessageSignature(t *testing.T) {
	...
	valid := CheckMessageSignature[hash.H256, uint64](grandpa.NewMessage(precommit), signedPrecommit.ID, signedPrecommit.Signature, 1, 1)
	require.True(t, valid)
	valid = CheckMessageSignature[hash.H256, uint64](grandpa.NewMessage(precommit), signedPrecommit.ID, signedPrecommit.Signature, 2, 1)
	...
	valid = CheckMessageSignature[hash.H256, uint64](grandpa.NewMessage(precommit), signedPrecommit.ID, signedPrecommit.Signature, 3, 3)
	require.True(t, valid)
	valid = CheckMessageSignature[hash.H256, uint64](grandpa.NewMessage(precommit), ed25519.Bob.Pair().Public().(ced25519.Public), signedPrecommit.Signature, 3, 3)
	require.True(t, valid)
	valid = CheckMessageSignature[hash.H256, uint64](grandpa.NewMessage(precommit), ed25519.Alice.Pair().Public().(ced25519.Public), signedPrecommit.Signature, 3, 3)
	require.False(t, valid)
}
