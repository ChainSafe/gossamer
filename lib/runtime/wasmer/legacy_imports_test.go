...
func TestExt_local_storage_set_local(t *testing.T) {
	...
	// call wasm function
	testFunc, ok := inst.vm.Exports["test_ext_local_storage_set"]
	if !ok {
		t.Fatal("could not find exported function")
	}
	...
	resValue, err := inst.ctx.NodeStorage.LocalStorage.Get(key)
	require.NoError(t, err)
...
func TestExt_local_storage_set_persistent(t *testing.T) {
	...
	resValue, err := inst.ctx.NodeStorage.PersistentStorage.Get(key)
	require.NoError(t, err)
	require.Equal(t, value, resValue)
}
...
func TestExt_local_storage_get_local(t *testing.T) {
	...
	key := []byte("mykey")
	value := []byte("myvalue")
	inst.ctx.NodeStorage.LocalStorage.Put(key, value)
	...
	// call wasm function
	testFunc, ok := inst.vm.Exports["test_ext_local_storage_get"]
	if !ok {
		t.Fatal("could not find exported function")
	}
...
func TestExt_local_storage_get_persistent(t *testing.T) {
	...
	key := []byte("mykey")
	value := []byte("myvalue")
	inst.ctx.NodeStorage.PersistentStorage.Put(key, value)
...
func TestExt_is_validator(t *testing.T) {
	// test with validator
	inst := NewTestLegacyInstanceWithRole(t, runtime.TEST_RUNTIME, byte(4))
	// call wasm function
	testFunc, ok := inst.vm.Exports["test_ext_is_validator"]
	if !ok {
		t.Fatal("could not find exported function")
	}
	res, err := testFunc()
	...
	inst = NewTestLegacyInstanceWithRole(t, runtime.TEST_RUNTIME, byte(1))
	// call wasm function
	testFunc, ok = inst.vm.Exports["test_ext_is_validator"]
	if !ok {
		t.Fatal("could not find exported function")
...
func TestExt_network_state(t *testing.T) {
	...
	testFunc, ok := inst.vm.Exports["test_ext_network_state"]
	if !ok {
		t.Fatal("could not find exported function")
	}

	writtenOutPtr, err := inst.ctx.Allocator.Allocate(4)
	require.NoError(t, err)
	...
	expected := inst.ctx.Network.NetworkState()
