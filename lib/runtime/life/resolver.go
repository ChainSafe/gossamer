...
func ext_logging_log_version_1(vm *exec.VirtualMachine) int64 {
	...
	switch int(level) {
	case 0:
		logger.Critical(fmt.Sprintf("[ext_logging_log_version_1] target=%s message=%s", string(target), string(msg)))
	case 1:
		logger.Warn(fmt.Sprintf("[ext_logging_log_version_1] target=%s message=%s", string(target), string(msg)))
	case 2:
		logger.Info(fmt.Sprintf("[ext_logging_log_version_1] target=%s message=%s", string(target), string(msg)))
	case 3:
		logger.Debug(fmt.Sprintf("[ext_logging_log_version_1] target=%s message=%s", string(target), string(msg)))
	case 4:
		logger.Trace(fmt.Sprintf("[ext_logging_log_version_1] target=%s message=%s", string(target), string(msg)))
	default:
		logger.Error(fmt.Sprintf("[ext_logging_log_version_1] level=%d target=%s message=%s", level, string(target), string(msg)))
	}
...
func ext_misc_print_utf8_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_misc_print_utf8_version_1] utf8 data: 0x%x", data))
	return 0
}
...
func ext_misc_print_hex_version_1(vm *exec.VirtualMachine) int64 {
	...
	dataSpan := vm.GetCurrentFrame().Locals[0]
	data := asMemorySlice(vm.Memory, dataSpan)
	logger.Debug(fmt.Sprintf("[ext_misc_print_hex_version_1] data is: 0x%x", data))
	return 0
}

func ext_allocator_malloc_version_1(vm *exec.VirtualMachine) int64 {
	size := uint32(vm.GetCurrentFrame().Locals[0])
	logger.Trace(fmt.Sprintf("[ext_allocator_malloc_version_1] executing with size %d...", size))

	// Allocate memory
	res, err := ctx.Allocator.Allocate(size)
	if err != nil {
		logger.Error(fmt.Sprintf("[ext_allocator_malloc_version_1]: %s", err))
		panic(err)
	}
...
func ext_allocator_free_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Trace(fmt.Sprintf("[ext_allocator_free_version_1] executing at address %d...", addr))

	// Deallocate memory
	err := ctx.Allocator.Deallocate(addr)
	if err != nil {
		logger.Error(fmt.Sprintf("[ext_allocator_free_version_1]: %s", err))
		panic(err)
	}
...
func ext_hashing_blake2_256_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_blake2_256_version_1] data is 0x%x and hash is 0x%x", data, hash))
...
func ext_hashing_twox_128_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_twox_128_version_1] data is 0x%x and hash is 0x%x", data, hash))
...
func ext_hashing_twox_64_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_twox_64_version_1] data is 0x%x and hash is 0x%x", data, hash))
...
func ext_storage_set_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Info(fmt.Sprintf("[ext_storage_set_version_1] key 0x%x and value 0x%x", key, value))
...
func ext_storage_next_key_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_storage_next_key_version_1] key is 0x%x and next is 0x%x", key, next))
...
func ext_storage_read_version_1(vm *exec.VirtualMachine) int64 {
	...
	value := storage.Get(key)
	logger.Debug(fmt.Sprintf("[ext_storage_read_version_1] key 0x%x and value 0x%x", key, value))
...
func storageAppend(storage runtime.Storage, key, valueToAppend []byte) error {
	...
	if len(valueCurr) == 0 {
		...
		if err != nil {
			logger.Trace(fmt.Sprintf("[ext_storage_append_version_1] item in storage is not SCALE encoded, overwriting at key 0x%x", key))
...
func ext_storage_append_version_1(vm *exec.VirtualMachine) int64 {
	...
	key := asMemorySlice(vm.Memory, keySpan)
	logger.Debug(fmt.Sprintf("[ext_storage_append_version_1] key 0x%x", key))
...
func ext_trie_blake2_256_ordered_root_version_1(vm *exec.VirtualMachine) int64 {
	...
	for i, val := range v {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("[ext_blake2_256_enumerated_trie_root]: %s", err))
			return 0
		}
		logger.Trace(fmt.Sprintf("[ext_trie_blake2_256_ordered_root_version_1] key 0x%x and value 0x%x", key, val))
	...
	logger.Debug(fmt.Sprintf("[ext_trie_blake2_256_ordered_root_version_1] root hash: %s", hash))
...
func ext_storage_root_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_storage_root_version_1] root hash: %s", root))
...
func ext_crypto_ed25519_public_keys_version_1(vm *exec.VirtualMachine) int64 {
	...
	if err != nil {
		logger.Warn(fmt.Sprintf("[ext_crypto_ed25519_public_keys_version_1] error for id 0x%x: %s", id, err))
...
func ext_crypto_sr25519_generate_version_1(vm *exec.VirtualMachine) int64 {
	...
	ks, err := ctx.Keystore.GetKeystore(id)
	if err != nil {
		logger.Warn(fmt.Sprintf("[ext_crypto_sr25519_generate_version_1] error for id 0x%x: %s", id, err))
		return 0
	}
...
func ext_hashing_keccak_256_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_keccak_256_version_1] data 0x%x has hash %s", data, hash))
...
func ext_hashing_sha2_256_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_sha2_256_version_1] data 0x%x hash hash %x", data, hash))
...
func ext_hashing_blake2_128_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_blake2_128_version_1] data 0x%x has hash 0x%x", data, hash))
...
func ext_hashing_twox_256_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_hashing_twox_256_version_1] data 0x%x has hash %s", data, hash))
...
func ext_trie_blake2_256_root_version_1(vm *exec.VirtualMachine) int64 {
	...
	logger.Debug(fmt.Sprintf("[ext_trie_blake2_256_root_version_1] root hash: %s", hash))
	copy(memory[ptr:ptr+32], hash[:])
	return int64(ptr)
}
