...
func testGenerateProof(t *testing.T, entries []Pair, keys [][]byte, metrics Metrics) (
...
func TestLeafNodes_SameHash_DifferentPaths_GenerateAndVerifyProof(t *testing.T) {
	...
	metrics := NewMockMetrics(ctrl)
	previousCall := metrics.EXPECT().NodesAdd(uint32(1))
	metrics.EXPECT().NodesAdd(uint32(2)).After(previousCall)
