...
func (s *Service) VerifyBlockJustification(hash common.Hash, justification []byte) (round uint64, setID uint64, err error) {
	fj := Justification{}
	...
	if err != nil {
		return 0, 0, fmt.Errorf("checking if round and set id has finalised block: %w", err)
	}
	...
	for _, just := range fj.Commit.Precommits {
		...
		if !isDescendant {
			return 0, 0, ErrPrecommitBlockMismatch
		}
		...
		if !isInAuthSet(publicKey, auths) {
			return 0, 0, ErrAuthorityNotInSet
		}
