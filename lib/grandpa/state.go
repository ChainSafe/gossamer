...
type GrandpaState interface { //nolint
	...
	SetPrevotes(round, setID uint64, data []SignedVoteNew) error
	SetPrecommits(round, setID uint64, data []SignedVoteNew) error
	GetPrevotes(round, setID uint64) ([]SignedVoteNew, error)
	GetPrecommits(round, setID uint64) ([]SignedVoteNew, error)
}
