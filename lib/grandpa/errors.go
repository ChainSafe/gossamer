...
var (
	...
	ErrInvalidSignature = fmt.Errorf("signature is not valid")
	...
	ErrNoPreVotedBlock = fmt.Errorf("cannot get pre-voted block")
	...
	ErrPrecommitBlockMismatch = fmt.Errorf("precommit block is not descendant of committed block")

	// ErrAuthorityNotInSet is returned when a precommit within a justification is signed by a key not in the authority set
	ErrAuthorityNotInSet = fmt.Errorf("authority is not in set")

	errVoteToSignatureMismatch = fmt.Errorf("votes and authority count mismatch")
	errVoteBlockMismatch       = fmt.Errorf("block in vote is not descendant of previously finalised block")
	errVoteFromSelf            = fmt.Errorf("got vote from ourselves")
)
