...
func (ct *commitsTracker) add(commitMessage *CommitMessage) {
	...
	ct.cleanup()
	element := ct.linkedList.PushFront(commitMessage)
	// data = commitMessageMapData{
	// 	message: commitMessage,
	// 	element: element,
	// }
	ct.mapping[blockHash] = element
}
...
func (ct *commitsTracker) cleanup() {
	...
	oldestBlockHash := oldestElement.Value.(*CommitMessage).Vote.Hash
...
func (ct *commitsTracker) delete(blockHash common.Hash) {
	elem, has := ct.mapping[blockHash]
	if !has {
		return
	}

	ct.linkedList.Remove(elem)
...
func (ct *commitsTracker) getMessageForBlockHash(
	...
	elem, ok := ct.mapping[blockHash]
	if !ok {
		return nil
	}

	return elem.Value.(*CommitMessage)
}
...
func (ct *commitsTracker) forEach(f func(message *CommitMessage)) {
	for _, elem := range ct.mapping {
		f(elem.Value.(*CommitMessage))
	}
}
