...
type WarpSyncVerificationResult struct {
	...
	Header        types.Header
	Justification consensus_grandpa.GrandpaJustification[hash.H256, uint32, generic.Header[uint32, hash.H256, runtime.BlakeTwo256]]
	Completed     bool
}
...
func (w *WarpSyncProof) verify(
	...
	result := &VerifyResult{
		...
		consensus_grandpa.GrandpaJustification[hash.H256, uint32, generic.Header[uint32, hash.H256, runtime.BlakeTwo256]]{},
	}
...
func (p *WarpSyncProofProvider) Generate(start common.Hash) ([]byte, error) {
	...
	for _, blockNumber := range authoritySetChanges {
		...
		justification, err := consensus_grandpa.DecodeJustification[hash.H256, uint32, runtime.BlakeTwo256, generic.Header[uint32, hash.H256, runtime.BlakeTwo256]](encJustification)
		if err != nil {
			return nil, err
		}
	...
	if !limitReached {
		...
		justification, err := consensus_grandpa.DecodeJustification[hash.H256, uint32, runtime.BlakeTwo256, generic.Header[uint32, hash.H256, runtime.BlakeTwo256]](
			latestJustification,
		)
		if err != nil {
			return nil, err
