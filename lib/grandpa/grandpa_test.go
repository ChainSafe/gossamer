...
func TestGetDirectVotes(t *testing.T) {
	...
	for i, k := range kr.Keys {
		...
		if i < 5 {
			gs.prevotes.set(voter, &SignedVote{
				Vote: voteA,
			})
		} else {
			gs.prevotes.set(voter, &SignedVote{
...
func TestGetPossibleSelectedBlocks_OneThirdEquivocating(t *testing.T) {
	...
	for i, k := range kr.Keys {
		...
		if i < 3 {
			gs.prevotes.set(voter, svA)
		} else if i < 6 {
			gs.prevotes.set(voter, svB)
		} else {
...
func TestIsCompletable(t *testing.T) {
	...
	for i, k := range kr.Keys {
		...
		if i < 6 {
			...
			gs.precommits.set(voter, &SignedVote{
				Vote: *voteA,
			})
		} else {
			gs.prevotes.set(voter, &SignedVote{
...
func TestDeterminePreVote_WithPrimaryPreVote(t *testing.T) {
	...
	gs.prevotes.set(primary, &SignedVote{
		Vote: *NewVoteFromHeader(header),
	})

	pv, err := gs.determinePreVote()
	require.NoError(t, err)
	p := gs.prevotes.get(primary)
	require.NotNil(t, p)
	require.Equal(t, pv, &p.Vote)
}
...
func TestGrandpaServiceCreateJustification_ShouldCountEquivocatoryVotes(t *testing.T) {
	...
	for _, v := range fakeAuthorities {
		...
		previous := prevotes.get(vote.AuthorityID)
		if previous == nil {
			prevotes.set(vote.AuthorityID, vote)
			totalLegitVotes++
		} else {
