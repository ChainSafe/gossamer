...
func TestValidityAttestation(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "Implicit",
			enumValue: parachaintypes.Implicit(parachaintypes.ValidatorSignature{}),
			encodingValue: []byte{
				0x1,
				0x0,
				0x0,
		...
		{
			name:      "Explicit",
			enumValue: parachaintypes.Explicit(parachaintypes.ValidatorSignature{}),
			encodingValue: []byte{
				0x2,
				0x0,
				0x0,
	...
	for _, c := range testCases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			newDst := parachaintypes.NewValidityAttestation()
			err = scale.Unmarshal(bytes, &newDst)
			require.NoError(t, err)
...
func TestParachainInherents(t *testing.T) {
	...
	expectedParaInherentsbytes := []byte{
		0,
		0,
		0,
		197,
