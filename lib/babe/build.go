...
func (b *BlockBuilder) buildBlock(parent *types.Header, slot Slot, rt runtime.Instance) (*types.Block, error) {
	logger.Trace(fmt.Sprintf("build block with parent %s and slot %v", parent, slot))
	...
	logger.Trace(fmt.Sprintf("built block encoded inherents: %v", inherents))
...
func (b *BlockBuilder) buildBlockExtrinsics(slot Slot, rt runtime.Instance) []*transaction.ValidTransaction {
	...
	for !hasSlotEnded(slot) {
		...
		logger.Trace(fmt.Sprintf("build block, applying extrinsic %s", extrinsic))

		ret, err := rt.ApplyExtrinsic(extrinsic)
		if err != nil {
			logger.Warn(fmt.Sprintf("failed to apply extrinsic %s: %s", extrinsic, err))
			continue
		}
		...
		if err != nil {
			...
			if errors.Is(e.msg, errExhaustsResources) || errors.Is(e.msg, errInvalidTransaction) {
				...
				if err != nil {
					logger.Debug(fmt.Sprintf("failed to re-add transaction with hash %s to queue: %s", hash, err))
				}
			}
		}

		logger.Debug(fmt.Sprintf("build block applied extrinsic %s", extrinsic))
...
func (b *BlockBuilder) addToQueue(txs []*transaction.ValidTransaction) {
	for _, t := range txs {
		...
		if err != nil {
			logger.Trace(fmt.Sprintf("Failed to add transaction to queue: %s", err))
		} else {
			logger.Trace(fmt.Sprintf("Added transaction with hash %s to queue", hash))
		}
	}
}
