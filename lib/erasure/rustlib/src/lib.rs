...
pub extern "C" fn obtain_chunks(
    ...
    data: *const u8,
    len: usize,
    flattened_chunks: *mut *mut u8,
    flattened_chunks_len: *mut usize,
) -> *const i8 {
    ...
    let chunks: Vec<Vec<u8>> = shards
        .into_iter()
        .map(|w: WrappedShard| w.into_inner())
        .collect();
    let mut flattened: Vec<u8> = chunks
        .iter()
...
pub extern "C" fn reconstruct(
    n_validators: usize,
    flattened_chunks: *const u8,
    flattened_chunks_len: usize,
    chunk_size: usize,
    res_data: *mut *mut u8,
    res_len: *mut usize,
) -> *const i8 {
    let flattened_slice: &[u8] =
        unsafe { std::slice::from_raw_parts(flattened_chunks, flattened_chunks_len) };
