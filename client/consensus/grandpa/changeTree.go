...
// Roots returns the TreeRoots of each fork in the ChangeTree
// This is the equivalent of the slice in the outermost layer of the TreeRoots
func (ct *ChangeTree[H, N]) Roots() []*PendingChangeNode[H, N] {
	return ct.TreeRoots
}

// Import a new node into the TreeRoots.
//
...
// WARNING: some users of this method (i.e. consensus epoch changes TreeRoots) currently silently
...
// FinalizationResult Result of finalizing a node (that could be a part of the TreeRoots or not).
...
// FinalizeWithDescendentIf Finalize a root in the TreeRoots by either finalizing the node itself or a
// node's descendent that's not in the TreeRoots, guaranteeing that the node
...
func (ct *ChangeTree[H, N]) FinalizeWithDescendentIf(hash *H, number N, isDescendentOf IsDescendentOf[H], predicate func(*PendingChange[H, N]) bool) (*FinalizationResult[H, N], error) {
	...
	// Retain only TreeRoots that are descendents of the finalized block (this
