...
func main() {
 ...
 // 	}
	...
	testServiceConfigA := &p2p.ServiceConfig{
		BootstrapNodes: []string{
			ipfsAddr,
		},
		Port: 7001,
	...
	testServiceConfigB := &p2p.ServiceConfig{
		BootstrapNodes: []string{
			ipfsAddr,
		},
		Port: 7002,
	...
	sa, err := p2p.NewService(testServiceConfigA)
	if err != nil {
	...
	sb, err := p2p.NewService(testServiceConfigB)
	if err != nil {
		log.Fatalf("NewService error: %s", err)
	}
	...
	go func() {
		for {	
			...
			peerB, err := sa.DHT().FindPeer(sa.Ctx(), sb.Host().ID())
			if err != nil {
				log.Fatalf("could not find peer: %s", err)
			}

			msg := []byte("hello there\n")
			err = sa.Send(peerB, msg)
			if err != nil {
				log.Fatalf("Send error: %s", err)
			}

			peerA, err := sb.DHT().FindPeer(sb.Ctx(), sa.Host().ID())
			if err != nil {
				log.Fatalf("could not find peer: %s", err)
			}

			msg = []byte("hello again\n")
			err = sb.Send(peerA, msg)
			if err != nil {
				log.Fatalf("Send error: %s", err)
			}
