...
func NewSimulator(num int) (sim *Simulator, err error) {
	...
	bootNode, err := NewService(&Config{
		BootstrapNodes: nil,
		Port:           0,
		RandSeed:       0,
		NoBootstrap:    false,
	...
	if err != nil {
		log.Error("cannot start bootnode", "err", err)
	}

	sim.BootNode = bootNode

	bootAddr := fmt.Sprintf("/ip4/127.0.0.1/tcp/4000/ipfs/%s", bootNode.host.ID().Pretty())
