...
func (n *Node) Start(ctx context.Context, waitErrCh chan<- error) (startErr error) {
	...
	go func(cmd *exec.Cmd, nodeName string, waitErr chan<- error) {
		err = cmd.Wait()
		if err != nil {
			if ctx.Err() != nil {
				err = fmt.Errorf("%s: %w: %s", nodeName, ctx.Err(), err)
			} else {
				err = fmt.Errorf("%s encountered a runtime error: %w (command %s)", n, err, cmd)
			}
		}
		waitErr <- err
