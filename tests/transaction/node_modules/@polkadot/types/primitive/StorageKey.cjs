...
function decodeHashers(registry, value, hashers) {
  ...
  for (let i = 0; i < hashers.length; i++) {
    ...
    const decoded = canDecode ? registry.createTypeUnsafe((0, _index.getSiName)(registry.lookup, type), [value.subarray(offset + hashLen)]) : registry.createTypeUnsafe('Raw', [value.subarray(offset, offset + hashLen)]);
