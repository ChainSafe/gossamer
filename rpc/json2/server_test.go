...
func TestServiceP2P(t *testing.T) {
	...
	// Valid request
	err = exec(s, "P2PService.PeerCount", &ServiceRequest{1337}, &res)
	if err != nil {
		t.Fatalf("request execution failed: %s", err)
	}
	if res.Result != 1337 {
	...
	// Exepected to return error
	res = ServiceResponse{}
	err = exec(s, "P2PService.Fail", &ServiceRequest{1337}, &res)
	if err == nil {
		t.Fatalf("expected error to be thrown")
	} else if err.Error() != ErrResponse.Error() {
		t.Fatalf("unexpected error. got: %s expected: %s", err, ErrResponse)
...
func testHelperP2P() {
	...
	if err != nil {
		log.Printf("Could not start IPFS node: %s", err)
	}
	...
	if err != nil {
		log.Printf("NewService error: %s", err)
	}
