...
func TestDecodeCustom_DecodeBabePrimaryPreDigest(t *testing.T) {
	...
	decodedPreDigest := new(BabePrimaryPreDigest)

	err := scale.DecodeCustom(encoded, decodedPreDigest)
	require.NoError(t, err)
	require.Equal(t, expected, decodedPreDigest)
}
