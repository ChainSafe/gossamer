...
func TestService_CreateBlockResponse_MaxSize(t *testing.T) {
	s := newTestSyncer(t)
	addTestBlocksToState(t, network.MaxBlockResponseSize*2, s.blockState)
	...
	require.NoError(t, err)
	require.Equal(t, int(network.MaxBlockResponseSize), len(resp.BlockData))
	require.Equal(t, uint(1), resp.BlockData[0].Number())
	require.Equal(t, uint(128), resp.BlockData[127].Number())

	max := uint32(network.MaxBlockResponseSize + 100)
	req = &network.BlockRequestMessage{
		RequestedData: 3,
		StartingBlock: *start,
		Direction:     network.Ascending,
...
func TestService_CreateBlockResponse_StartHash(t *testing.T) {
	s := newTestSyncer(t)
	addTestBlocksToState(t, uint(network.MaxBlockResponseSize*2), s.blockState)
	...
	// test descending with nil endBlockHash and start > network.MaxBlockResponseSize
	...
	resp, err = s.CreateBlockResponse(req)
	require.NoError(t, err)
	require.Equal(t, network.MaxBlockResponseSize, len(resp.BlockData))
	require.Equal(t, uint(128), resp.BlockData[0].Number())
	require.Equal(t, uint(1), resp.BlockData[127].Number())
}
