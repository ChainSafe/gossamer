...
func (_m *TransactionStateAPI) FreeStatusNotifierChannel(ch chan transaction.StatusNotification) {
	_m.Called(ch)
}

// GetStatusNotifierChannel provides a mock function with given fields:
func (_m *TransactionStateAPI) GetStatusNotifierChannel() chan transaction.StatusNotification {
	ret := _m.Called()

	var r0 chan transaction.StatusNotification
	if rf, ok := ret.Get(0).(func() chan transaction.StatusNotification); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(chan transaction.StatusNotification)
		}
	}
