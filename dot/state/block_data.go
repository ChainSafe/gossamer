...
func (bs *BlockState) HasJustification(hash common.Hash) (bool, error) {
	return databaseHas(bs.db, prefixKey(hash, justificationPrefix))
}
...
func (bs *BlockState) SetJustification(hash common.Hash, data []byte) error {
	err := bs.db.Set(prefixKey(hash, justificationPrefix), data)
	if err != nil {
		return err
	}
