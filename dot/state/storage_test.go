...
func newTestStorageState(t *testing.T, tries *Tries, trieMetrics triemetrics.Metrics) *StorageState {
	db := NewInMemoryDB(t)

	bs := newTestBlockState(t, testGenesisHeader, tries)

	s, err := NewStorageState(db, bs, tries, pruner.Config{}, trieMetrics)
...
func TestStorage_StoreAndLoadTrie(t *testing.T) {
	...
	storage := newTestStorageState(t, newTriesEmpty(), trieMetrics)
...
func TestGetStorageChildAndGetStorageFromChild(t *testing.T) {
	...
	trieMetrics := triemetrics.NewNoop()
	_, genTrie, genHeader := genesis.NewTestGenesisWithTrieAndHeader(t, trieMetrics)
	...
	testChildTrie := trie.NewEmptyTrie(trieMetrics)
	...
	storage, err := NewStorageState(db, blockState, tries, pruner.Config{}, trieMetrics)
	require.NoError(t, err)
