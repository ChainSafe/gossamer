...
func readStream(stream libp2pnetwork.Stream, buf []byte) (int, error) {
	...
	if length > uint64(len(buf)) {
		logger.Warn(fmt.Sprintf("received message with size %d greater than allocated message buffer size %d", length, len(buf)))
		return 0, fmt.Errorf("message size greater than allocated message buffer: got %d", length)
	}

	if length > maxBlockResponseSize {
		logger.Warn(fmt.Sprintf("received message with size %d greater than maxBlockResponseSize, closing stream", length))
