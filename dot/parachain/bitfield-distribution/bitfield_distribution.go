...
type perRelayParentData struct {
	...
	validatorsSet []parachaintypes.ValidatorPK
	...
	onePerValidator map[parachaintypes.ValidatorPK]*validationprotocol.BitfieldDistributionMessage

	// Avoid duplicate message transmission to our peers.
	messageSentToPeer map[peer.ID]map[parachaintypes.ValidatorPK]struct{}

	// Track messages that were already received by a peer to prevent flooding.
	messageReceivedFromPeer map[peer.ID]map[parachaintypes.ValidatorPK]struct{}
}

// this will be used in the PeerViewChange handler so skip the lint check for now
//
//nolint:all
func newPerRelayParentData(sessionIndex parachaintypes.SessionIndex, validatorSet []parachaintypes.ValidatorPK,
) *perRelayParentData {
	return &perRelayParentData{
		sessionIndex:            sessionIndex,
		validatorsSet:           validatorSet,
		onePerValidator:         make(map[parachaintypes.ValidatorPK]*validationprotocol.BitfieldDistributionMessage),
		messageSentToPeer:       make(map[peer.ID]map[parachaintypes.ValidatorPK]struct{}),
		messageReceivedFromPeer: make(map[peer.ID]map[parachaintypes.ValidatorPK]struct{}),
	}
}
...
func (p *perRelayParentData) messageFromValidatorNeededByPeer(
	peerID peer.ID,
	signedBy parachaintypes.ValidatorPK,
) bool {
...
func (b *BitfieldDistribution) processIncomingPeerMessageEvent(event networkbridgeevents.PeerMessage[validationprotocol.
	...
	if receivedSet == nil {
		receivedSet = make(map[parachaintypes.ValidatorPK]struct{})
...
func relayMessage(
	...
	for peerID := range interestedPeers {
		jobData.messageSentToPeer[peerID] = map[parachaintypes.ValidatorPK]struct{}{validatorID: {}}
	}
...
func (b *BitfieldDistribution) sendTrackedGossipMessage(
	...
	if jobData.messageSentToPeer[dest] == nil {
		jobData.messageSentToPeer[dest] = map[parachaintypes.ValidatorPK]struct{}{
			validatorId: {},
		}
	}
