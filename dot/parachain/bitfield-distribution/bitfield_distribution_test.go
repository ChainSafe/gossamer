...
func TestMessageFromValidatorNeededByPeer(t *testing.T) {
	validatorSet := []parachaintypes.ValidatorPK{
	...
	pretendSent := func(state *perRelayParentData, destPeer peer.ID, signedBy parachaintypes.ValidatorPK) bool {
		if state.messageFromValidatorNeededByPeer(destPeer, signedBy) {
			state.messageSentToPeer[destPeer] = map[parachaintypes.ValidatorPK]struct{}{signedBy: {}}
			return true
		} else {
			return false
		}
	...
	pretendReceive := func(state *perRelayParentData, sourcePeer peer.ID, signedBy parachaintypes.ValidatorPK) {
		state.messageReceivedFromPeer[sourcePeer] = map[parachaintypes.ValidatorPK]struct{}{signedBy: {}}
	}
...
func prepareForValidators() (parachaintypes.ValidatorIndex, []parachaintypes.ValidatorPK) {
...
func TestBitfieldDistribution_ProcessBitfieldDistributionMessage_CheckSignedAvailabilityBitfield(t *testing.T) {
	...
	validatorSet := []parachaintypes.ValidatorPK{
		parachaintypes.ValidatorPK(aliceKeypair.Public().Encode()),
		[sr25519.PublicKeyLength]byte{2},
	}
	jobData := newPerRelayParentData(parachaintypes.SessionIndex(1), validatorSet)
	peerViews := map[peer.ID]*networkbridge.PeerDataViewWithVersion{}
...
func TestBitfieldDistribution_ProcessIncomingPeerMessageEvent_DuplicatedMessages(t *testing.T) {
	...
	jobData.onePerValidator = map[parachaintypes.ValidatorPK]*validationprotocol.BitfieldDistributionMessage{
