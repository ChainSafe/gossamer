...
// 		trieDB := NewTrieDB[hash.H256, hasher.Blake2Hasher](root, db, version)
// 		b.ResetTimer()
// 		for i := 0; i < b.N; i++ {
...
// 		trieDB := NewTrieDB(
// 			root, db, version, WithCache[hash.H256, hasher.Blake2Hasher](cache))
// 		b.ResetTimer()
// 		for i := 0; i < b.N; i++ {
// 			// Use the deepest key to ensure the trie is traversed fully
// 			val, err := GetWith(trieDB, []byte("notification"), func(d []byte) []byte { return d })
...
// 		cache := NewTestTrieCache[hash.H256]()
// 		trieDB := NewTrieDB(root, db, version, WithCache[hash.H256, hasher.Blake2Hasher](cache))
// 		b.ResetTimer()
// 		for i := 0; i < b.N; i++ {
// 			// Iterate through all keys
// 			iter, err := NewTrieDBRawIterator(trieDB)
