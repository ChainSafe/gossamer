...
func TestParachainHeaderStateProof(t *testing.T) {
	...
	trie, err := buildTrie(proofDB, stateRoot)
	require.NoError(t, err)
...
func TestTrieProof(t *testing.T) {
	...
	require.NoError(t, err)
	t.Logf("0x%x", codec.KeyLEToNibbles(key))
	...
	buffer := bytes.NewBuffer(proof1)
	proof1Node, err := node.Decode(buffer)
	require.NoError(t, err)
	...
	buffer = bytes.NewBuffer(proof2)
	proof2Node, err := node.Decode(buffer)
	require.NoError(t, err)
	...
	buffer = bytes.NewBuffer(proof3)
	proof3Node, err := node.Decode(buffer)
	require.NoError(t, err)
	...
	trie, err := buildTrie(proofDB, root)
	require.NoError(t, err)
	t.Log(trie.(*inmemory.InMemoryTrie).RootNode())
